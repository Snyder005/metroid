...
| ---- | ---- | ------------ |
| `throughput.py` | `ThroughputCurve` class wrapping a `speclite` `FilterResponse`; methods to calculate photon flux, energy flux, ADU, and AB magnitude from an SED or AB magnitude float, plus `calculate_ab_magnitude_from_adu` (the ADU→magnitude inverse of `calculate_adu`) | Implementing bandpass convolution; debugging photon/energy flux, ADU, or ADU→magnitude calculations; adding filter loading |
