...
| `config.py` | `load_yaml` YAML→dict loader; `load_standard_catalogue` resolving the bundled `metroid/data/standard_objects.yaml` via `importlib.resources`; `Registrable` mixin providing the per-base subclass registry + `from_config`/`_from_config` type dispatch shared by `Pupil`, `Camera`, `Observatory` | Implementing declarative construction from config/YAML; adding a `from_config` to a class; debugging registry dispatch or catalogue resolution |
